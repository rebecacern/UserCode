...
void chain(int nsel = 0, int mh = 125){
  ...
  TH1F* histo = new TH1F( title, " ", 200, 0, 400);
  histo->Sumw2();
  ...
  int nSample=sample.tree_->GetEntries();
   ...
   //Three real leptons MC level
    ...
    double mt = 0;
    double dR = 0;
    if (min == fabs(mz - m[0])) {  pair = pair1; mt =  sample.mt3_; tlepton = sample.lep3_; dR = fabs(ROOT::Math::VectorUtil::DeltaR(sample.lep1_ ,sample.lep2_));} 
    else if (min == fabs(mz - m[1])){  pair = pair2;  mt =  sample.mt1_; tlepton = sample.lep1_; dR = fabs(ROOT::Math::VectorUtil::DeltaR(sample.lep2_ ,sample.lep3_));} 
    else if (min == fabs(mz - m[2])){  pair = pair3;  mt =  sample.mt2_; tlepton = sample.lep2_; dR = fabs(ROOT::Math::VectorUtil::DeltaR(sample.lep1_ ,sample.lep3_));} 
  ...
  cout << plotName << ": " <<  eventsPass << " events pass " << endl;
