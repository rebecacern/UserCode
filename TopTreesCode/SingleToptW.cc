...
int main() {
 ...
 double lumi = 1000;
  ...
  char name[100];
  double weight;
  if (sample == 0){		sprintf(name, "tt");			weight = lumi*157.5/1284848;}
  else if (sample == 1){	sprintf(name, "tw");			weight = lumi*10.6/489417;}
  else if (sample == 2){	sprintf(name, "t");			weight = lumi*20.93/484060;}
  else if (sample == 3){	sprintf(name, "s");			weight = lumi*4.6/494967;}
  ...
  sprintf(rootFileName,"outputs/out_tc_%d_%s.root", mode, name);
		  ...
		  if (leptonVeto) {
			...
			for (int i =0; i < selectedJets.size(); i ++){
			  ...
			  if (tempJet->Pt() > 30 && TMath::Min(fabs(lepton0.DeltaR(tJet)), fabs(lepton1.DeltaR(tJet))) > 0.3) {
			    nJets++;
			    iJet = i;
			    if (tempJet->btag_simpleSecondaryVertexHighEffBJetTags() > 1.74) bTagged = true;
			  }
			}
			if(nJets == 1){
